...
def check():
    ...
    res = requests.get(f"{BASE_URL}/api/ruleChains?pageSize=100&page=0", headers=tenant_headers)
    rule_chains = res.json()['data']
    for rc in rule_chains:
        if rc.get('root'):
            print(f"Root Rule Chain ID: {rc['id']['id']}")
            res = requests.get(f"{BASE_URL}/api/ruleChain/{rc['id']['id']}/metadata", headers=tenant_headers)
